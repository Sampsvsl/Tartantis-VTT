...
def _scenes_dir(code: str) -> Path:
    return ROOMS_DIR / code / 'scenes'

def _scene_file(code: str, scene_id: str) -> Path:
    return _scenes_dir(code) / f'{scene_id}.json'

def _list_scenes(code: str) -> list:
    d = _scenes_dir(code)
    if not d.exists():
        return []
    scenes = []
    for f in sorted(d.glob('*.json'), key=lambda x: x.stat().st_mtime):
        try:
            ...
            # Retorna versão resumida (sem tokens e walls para economizar banda)
            scenes.append({
                'id':        s.get('id', f.stem),
                'name':      s.get('name', 'Cena'),
                'bgImageURL': s.get('bgImageURL', ''),
    ...
    def _room_get(self, path: str):
        ...
        if resource == 'char' and len(parts) == 5:
            ...
            if len(parts) == 4:
                ...
                data = _read_json(_scene_file(code, scene_id))
                if data is None:
                    self._json_error(404, 'Cena não encontrada')
                else:
                    self._json_response(200, json.dumps(data, ensure_ascii=False))
            else:
                self._json_error(404, 'not found')
